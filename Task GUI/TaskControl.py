...
class TaskControl():
        ...
        if params is not None:
            ...
            if 'configPath' in params:
                ...
                if self.rigName in ('NP1','NP2','NP3'):
                    ...
                    soundFilterPath = r"C:\\Users\\teenspirit\\Desktop\\Behavior\\Tilda\\Speaker calibrations\\Tildas speaker calibration 01252024\\01252024_npx_spkrleft_31-80k_fs200k.mat"
                    import scipy.io
    ...
    #                 print("ABOUT TO SAVE ROTARY ENCODER")
    #                 print("Number of encoder samples:", len(self.rotaryEncoderCount))
    #                 print("First 20:", self.rotaryEncoderCount[:20])
    #                 print("Last 20:", self.rotaryEncoderCount[-20:])
    ...
    def completeSession(self):
        try:
            ...
            print("ERROR during session cleanup:", repr(e))
            ...
            if self.saveParams:
                ...
                if self.saveDir is not None:
                    ...
                    self.savePath = os.path.join(
                        saveDir,
                        self.__class__.__name__ + '_' + subjName + self.startTime + '.hdf5'
                    )
                ...
                try:
                    with h5py.File(self.savePath, 'w') as fileOut:
                        ...
                        if self.saveFrameIntervals and self._win is not None:
                            try:
                                fileOut.create_dataset(
                                'frameIntervals',
                                data=self._win.frameIntervals
                                )
                            except Exception as e:
                                print("Could not save frame intervals:", repr(e))

                    print("HDF5 SAVE SUCCESSFUL")

                except Exception as e:
                    print("ERROR SAVING HDF5:", repr(e))
    ...
    def initDigitalEncoder(self):
        self._digitalEncoder = serial.Serial(
            port=self.rotaryEncoderSerialPort,
            baudrate=9600,
            timeout=0.5
        )
        ...
        self._digitalEncoder.reset_input_buffer()

        # The Arduino clears the encoder whenever it receives a serial byte.
        # Send one byte to reset the encoder count.
        self._digitalEncoder.write(b'2')

        # Give Arduino time to process the reset
        time.sleep(0.05)

        # Discard values generated during initialization
        self._digitalEncoder.reset_input_buffer()

        print(
            'Digital rotary encoder initialized on',
            self.rotaryEncoderSerialPort
        )

    def readDigitalEncoder(self):
        try:
            r = self._digitalEncoder.readline().decode('utf-8').strip()

            if r:
                count = int(r)
                self.rotaryEncoderCount.append(count)
    ...
    def initSolenoid(self):
        ...
        if self.digitalSolenoidTrigger:
            self._solenoid.do_channels.add_do_chan(
                self.behavNidaqDevice + '/port0/line7',
                line_grouping=nidaqmx.constants.LineGrouping.CHAN_PER_LINE
            )
        else:
            self._solenoid.ao_channels.add_ao_voltage_chan(
                self.nidaqDeviceNames[0] + '/ao0',
                min_val=0,
                max_val=5
            )
